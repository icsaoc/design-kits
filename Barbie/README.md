...
- **Library source:** generation summary (not included in this repo)
- **Kit type:** `dashboard`
...
- **Foundations** — light and dark color tokens, font-family tokens, a single oversized radius token, 4px spacing base, tracking, and a pink-tinted elevation system documented in `colors_and_type.css` and `css.json`
- **Components** — 6 documented component families in `components/`: button, card, table, chart, navigation, and sidebar
- **Preview layer** — 6 HTML previews in `preview/` for quick visual reference
...
- The kit ships a 40-icon SVG set in `assets/icons/` — a single-weight, `currentColor` outline family that inherits token color from its container.
...
Across components, three patterns recur in `components/index.json`: a single oversized radius token that keeps every silhouette soft, primary actions that rely on saturated pink with white text, and data surfaces that choose blush neutrals plus pink-tinted shadows instead of grayscale enterprise chrome. That combination is what makes the dashboard feel recognizably Barbie without losing operational clarity.

## Generated file index

- `README.md` — this file
- `colors_and_type.css` — combined CSS token file for color, type, spacing, radius, tracking, and shadows
- `css.json` — structured JSON token export with hex values and shadow primitives
- `components/index.json` — component index and cross-component patterns
- `components/button.json` — button anatomy, variables, and variant dimensions
- `components/card.json` — card anatomy, variables, and variant dimensions
- `components/chart.json` — chart component analysis
- `components/navigation.json` — top navigation analysis
- `components/sidebar.json` — sidebar analysis
- `components/table.json` — table analysis
- `preview/component-button.html` — button preview page
- `preview/component-card.html` — card preview page
- `preview/component-chart.html` — chart preview page
- `preview/component-navigation.html` — navigation preview page
- `preview/component-sidebar.html` — sidebar preview page
- `preview/component-table.html` — table preview page

## Caveats / known substitutions

1. **Font scale data is incomplete.** `css.json` includes family tokens for Inter, Georgia, and JetBrains Mono, but it does not include size, weight, or line-height tokens. Any detailed typography scale in implementation is therefore inferred, not directly observed.
...
5. **Icon guidance is asset-backed but not tokenized.** `assets/icons/` ships 40 outline SVGs, but the token export defines no icon-specific variables, so size and color decisions belong to the consuming component.
6. **The library output is structurally partial.** The output spec describes `SKILL.md` and `ui_kits/<type>/index.html` as part of a complete design library, but neither exists in this kit at the time of writing.
