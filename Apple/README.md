...
**Pinguo** is the fictional product this kit reconstructs: a consumer imaging ecosystem shaped around
creation, merchandising, and device-like product storytelling. It is filed in this repository as `Apple/`
because the visual language it reconstructs is premium-retail — restrained neutrals, whitespace as
structure, merchandising rhythm.

This is an independent reconstruction. It is not affiliated with, endorsed by, or an official release
from Apple Inc., and it is not built from any official Apple design resource. See
[NOTICE](../NOTICE.md).

> *"Content first, whitespace is design, restraint is style."*

### What this covers
- **Foundations** — a restrained palette of premium-retail neutrals, organized into six token categories (`brand`, `background`, `text`, `icon`, `state-success`, `state-error`); DM Sans and JetBrains Mono; a `0.24rem` spacing base; a global radius token of `1.2rem`; and quiet shadow variables from `shadow-2xs` through `shadow-2xl`
...
- Cards and hero media containers step into medium-to-large rounding, reinforcing the premium-hardware softness without becoming playful.
...
Reconstructed from a structured-spec bundle. The source package did not carry a page count, an original
file name, or brand-owner attribution; "Pinguo" is the product name used throughout this reconstruction,
not a real company.
