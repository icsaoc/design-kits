...
The kit ships a 40-icon SVG set in `assets/icons/` — a single-weight, `currentColor` outline family that inherits token color from its container. The token export defines no icon-specific variables, so sizing and color come from the surrounding component rather than from a dedicated icon token family.
